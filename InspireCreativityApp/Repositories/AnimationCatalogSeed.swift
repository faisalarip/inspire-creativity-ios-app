...
enum AnimationCatalogSeed {
    ...
    static let handcrafted: [AnimationItem] = [
        ...
        .init(
            ...
            author: "InspireCreativity", handle: "@inspirecreativity",
    ...
    static let auroraDerived: [AnimationItem] = AuroraDescriptors.all.map { d in
        ...
        // Respect the descriptor's authored Pro flag. (Previously this also
        // required price >= 10, which silently turned every sub-$10 Pro
        // background into a free item — giving the whole catalog away.)
        let keepPro = d.isPro

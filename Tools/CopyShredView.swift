...
// sliding in. demo == true self-cycles via PhaseAnimator; demo == false fires
// on tap and auto-resets so it stays re-tappable.
...
private struct DemoDriver: View {
    ...
    var body: some View {
        // Trigger-less PhaseAnimator auto-advances and loops forever:
        //   0 (idle "Copy") → 1 (reformed "Copied" + tick) → 0 (reset) → ...
        PhaseAnimator([CGFloat(0), CGFloat(1), CGFloat(0)]) { phase in
            ShredContent(progress: phase, size: size, animated: false)
        } animation: { phase in
            switch phase {
            case 1:  return .easeInOut(duration: 1.4)          // shred → reform
            default: return .easeInOut(duration: 1.0).delay(1.1) // hold then reset
            }
        }
    }
}
...
private struct ShredContent: View {
    ...
    private var checkmark: some View {
        ...
        return Image(systemName: "checkmark.circle.fill")
            ...
            .foregroundStyle(Color(hex: 0x8BE6B0))
            .opacity(Double(p))
...
private extension View {
    ...
    /// kept behind a tiny wrapper for clarity.
    @ViewBuilder
